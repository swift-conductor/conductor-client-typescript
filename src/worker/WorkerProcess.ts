...
import { Task, TaskResourceApi, TaskResult, TaskResultStatusEnum } from "../../openapi/api";
...
import { WorkerErrorHandler, WorkerProcessOptions, WorkerProcessConfig } from "./types";
...
export const noopErrorHandler: WorkerErrorHandler = (__error: Error) => { };

const defaultRunnerOptions: Required<WorkerProcessOptions> = {
    workerID: "",
...
export class WorkerProcess {
    ...
    startPolling = () => {
        ...
        this.logger.info(`TaskWorker ${this.worker.taskDefName} initialized with concurrency of ${this.poller.options.concurrency} and poll interval of ${this.poller.options.pollInterval}`);
    };
    ...
    updateOptions(options: Partial<WorkerProcessOptions>) {
        const newOptions = { ...this.options, ...options };
        const isOptionsUpdated = !optionEquals(this.options, newOptions);

        if (isOptionsUpdated) {
            this.poller.updateOptions({ concurrency: newOptions.concurrency, pollInterval: newOptions.pollInterval });

            this.logger.info(`TaskWorker ${this.worker.taskDefName} configuration updated with concurrency of ${this.poller.options.concurrency} and poll interval of ${this.poller.options.pollInterval}`);
        }

        this.options = newOptions;
    }

    get getOptions(): WorkerProcessOptions {
        return this.options;
    }
    ...
    updateTaskWithRetry = async (task: Task, taskResult: TaskResult) => {
        ...
        while (retryCount < MAX_RETRIES) {
            try {
                await this.taskResource.updateTask({...taskResult, workerId: workerID});
                return;
            } catch (error: unknown) {
